...
export function useProposals(args: ProposalsRequest, shouldFetch: boolean = true) {
  ...
  return useSWR<APIResponse<ProposalsPacket>, string>(
    shouldFetch ? `${apiUrl}/${args.space}/proposals?` + urlParams.toString() : null,
    jsonFetcher(),
  );
}
...
export async function fetchCreatedProposals(apiUrl: string, space: string | undefined, author: string | undefined, prefix: string = "") {
  if (!space || !author) {
    return {
      success: true,
      data: {
